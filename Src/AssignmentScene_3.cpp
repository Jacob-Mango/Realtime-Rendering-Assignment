...
void AssignmentScene_3::OnStart()
{
	Scene::OnStart();
	...
	int drawMethod = g_SceneFlags & USE_DYNAMIC_DRAW ? GL_DYNAMIC_DRAW : GL_STATIC_DRAW;

	glGenBuffers(1, &m_VertexBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, m_VertexBufferObject);
	glBufferData(GL_ARRAY_BUFFER, SIZE_OF(m_Positions, glm::vec3) + SIZE_OF(m_Normals, glm::vec3) + SIZE_OF(m_Materials, unsigned int), NULL, drawMethod);

	glGenBuffers(1, &m_IndexBufferObject);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IndexBufferObject);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, SIZE_OF(m_Indices, unsigned int), m_Indices.data(), drawMethod);
...
void AssignmentScene_3::DestroyModel()
{
	m_Transformations.clear();
}

void AssignmentScene_3::GenerateTransformation(Cube* cube)
{
...
void AssignmentScene_3::CreateModel()
{
	Scene::CreateModel();
...
void AssignmentScene_3::OnSimulate(double DeltaTime)
{
	Scene::OnSimulate(DeltaTime);
}

void AssignmentScene_3::OnRender(double DeltaTime)
{
	m_Shader->Bind();
	...
	for (int i = 0; i < m_Transformations.size(); i++)
	{
		...
		m_Shader->Set("u_ModelMatrix", transform);
...
std::string AssignmentScene_3::Name()
{
	return "Scene 3";
}
