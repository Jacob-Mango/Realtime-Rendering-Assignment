...
void AssignmentScene_6::OnStart()
{
	Scene::OnStart();

	SpawnEntity<Camera>(g_CameraType);

	m_Shader = new Shader("shaders/morph.vs", "shaders/lighting.fs");

	CreateModel();
}

void AssignmentScene_6::OnStop()
{
	m_Shader = nullptr;
	DestroyModel();
...
void AssignmentScene_6::GenerateMesh(Cube* cube, int& offset)
{
...
void AssignmentScene_6::CreateModel()
{
	Scene::CreateModel();
	...
	auto CubeToSphere = [](glm::vec3 point) {
		const float x2 = point.x * point.x;
		const float y2 = point.y * point.y;
		const float z2 = point.z * point.z;
		glm::vec3 result = glm::vec3(0);
		result.x = point.x * sqrtf(1.0f - ((y2 + z2) * 0.5f) + (y2 * z2 * 0.33333333333333333333f));
		result.y = point.y * sqrtf(1.0f - ((z2 + x2) * 0.5f) + (z2 * x2 * 0.33333333333333333333f));
		result.z = point.z * sqrtf(1.0f - ((x2 + y2) * 0.5f) + (x2 * y2 * 0.33333333333333333333f));
		return result;
	};

	for (int i = 0; i < m_Positions.size(); i++)
	{
		m_Target.push_back(glm::mix(m_Positions[i], CubeToSphere(m_Positions[i]), glm::pi<double>()));
	}
	...
	glBufferData(GL_ARRAY_BUFFER, SIZE_OF(m_Positions, glm::vec3) + SIZE_OF(m_Normals, glm::vec3) + SIZE_OF(m_Materials, unsigned int) + SIZE_OF(m_Target, glm::vec3), NULL, GL_STATIC_DRAW);
	...
	glBufferSubData(GL_ARRAY_BUFFER, SIZE_OF(m_Positions, glm::vec3) + SIZE_OF(m_Normals, glm::vec3) + SIZE_OF(m_Materials, unsigned int), SIZE_OF(m_Target, glm::vec3), m_Target.data());
	...
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), (void*)(SIZE_OF(m_Positions, glm::vec3) + SIZE_OF(m_Normals, glm::vec3) + SIZE_OF(m_Materials, unsigned int)));
...
void AssignmentScene_6::DestroyModel()
{
	Scene::DestroyModel();
...
void AssignmentScene_6::OnSimulate(double DeltaTime)
{
	Scene::OnSimulate(DeltaTime);
}

void AssignmentScene_6::OnRender(double DeltaTime)
{
	m_Shader->Bind();
	...
	auto cubicEasing = [](double t) { return (t < 0.5) ? 4 * t * t * t : (t - 1) * (2 * t - 2) * (2 * t - 2) + 1; };

	m_Shader->Set("u_Time", (float)cubicEasing(m_Time));
...
std::string AssignmentScene_6::Name()
{
	return "Scene 6";
}
