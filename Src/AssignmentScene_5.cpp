...
void AssignmentScene_5::OnStart()
{
	Scene::OnStart();
...
void AssignmentScene_5::GenerateMesh(Cube* cube, int& offset)
{
...
void AssignmentScene_5::CreateModel()
{
	Scene::CreateModel();
	...
	{
		m_Source.push_back(m_Positions[i]);
...
void AssignmentScene_5::DestroyModel()
{
	Scene::DestroyModel();
...
void AssignmentScene_5::OnSimulate(double DeltaTime)
{
	Scene::OnSimulate(DeltaTime);
}

void AssignmentScene_5::OnRender(double DeltaTime)
{
	m_Shader->Bind();
	...
	double easedTime = cubicEasing(m_Time);

	for (int i = 0; i < m_Positions.size(); i++)
	{
		m_Positions[i] = glm::mix(m_Source[i], m_Target[i], easedTime);
	}
...
std::string AssignmentScene_5::Name()
{
	return "Scene 5";
}
